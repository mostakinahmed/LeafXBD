...
export const MyOrder = () => {
  ...
  // --- FIX 1: Prevent Background Scroll Jump ---
  useEffect(() => {
    if (selectedOrder) {
  ...
  return (
    <div className="animate-in slide-in-from-bottom-2 duration-300 pb-7 md:mb-0">
          ...
          >
            <table className="w-full text-left border-collapse whitespace-nowrap font-sans">
              <thead className="bg-slate-50 border-b">
                <tr>
                  <th className="md:px-6 px-4 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    Order ID
                  </th>
                  <th className="md:px-6 px-4 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    Date
                  </th>
                  <th className="md:px-6 px-2 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center">
                    Tracking
                  </th>
                  <th className="md:px-6 px-2 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    Total
                  </th>
                  <th className="md:px-6 px-2 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center">
                    Actions
                  ...
                  >
                    ...
                    <td className="md:px-6 px-4 py-3">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[12px] uppercase font-medium text-slate-600 tracking-tighter">
                    ...
                    <td className="md:px-6 px-4 py-3 text-sm font-medium text-gray-800">
                      ৳{order.total_amount}
                    </td>
                    <td className="md:px-6 px-2 py-3 text-center">
                      <button
      ...
      {/* --- FIX 2: Render Modal via Portal --- */}
      {createPortal(
        <AnimatePresence>
          {selectedOrder && (
            <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 backdrop-blur-sm bg-slate-900/20">
              <motion.div
                initial={{ opacity: 0, scale: 0.95, y: 10 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 10 }}
                transition={{ duration: 0.2 }}
                className="bg-white w-full max-w-3xl max-h-[90vh] border-1 border-[#1976d2] overflow-y-auto rounded-xl shadow-2xl relative scrollbar-thin scrollbar-thumb-slate-200"
              >
                {/* Modal Header */}
